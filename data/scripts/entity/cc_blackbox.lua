...
function CosmicChroniclesBlackBox.initUI()
    ScriptUI():registerInteraction("Extract Data", "onInteract")
end
...
function CosmicChroniclesBlackBox.extract()
    ...
    local log = CosmicVaultDialogue.getValidLine("captain_log", context) or "Mayday! Shields are failing! They're coming from everywhere!"
    local text = "Black Box Recording:\n\n\"" .. log .. "\"\n\n*Recording Ends*"

    invokeClientFunction(player, "showLogDialog", text)
...
function CosmicChroniclesBlackBox.showLogDialog(text)
    if not onClient() then return end
    local dialog = {text = text, answers = {{answer = "Close"}}}
