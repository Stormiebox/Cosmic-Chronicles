...
local function normalizeConditions(source)
    local result = {}
    for key, value in pairs(source or {}) do
        if key == "stationType" then
            result.stationTypes = {[tostring(value)] = true}
        elseif key == "factionTrait" then
            result.factionTraits = {[tostring(value)] = true}
        elseif key == "factionWealth" then
            result.factionWealth = {[tostring(value)] = true}
        else
            result[key] = value
        end
    end
